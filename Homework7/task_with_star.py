...
class RomanNums:
    def __init__(self, roman_num):
        ...
        self.roman_numbers = {'M': 1000, 'CM': 900, 'D': 500, 'CD': 400, 'C': 100, 'XC': 90, 'L': 50, 'XL': 40, 'X': 10,
                         'IX': 9, 'VI': 6, 'V': 5, 'IV': 4, 'I': 1, }
    def from_roman(self):
        result = 0
        i = 0
        while i < len(self.roman_num):
            if i + 1 < len(self.roman_num) and self.roman_numbers[self.roman_num[i]] < self.roman_numbers[self.roman_num[i + 1]]:
                result -= self.roman_numbers[self.roman_num[i]]
            else:
                result += self.roman_numbers[self.roman_num[i]]
            i += 1
    ...
    def is_palindrome(self):
        str_roman = str(self.from_roman())
        return str_roman == str_roman[::-1]
